using Chatty.ApplicationCore.Entities;
using Chatty.ApplicationCore.Interfaces.Repositories;
using Chatty.ApplicationCore.Utilities;
using Chatty.Infrastructure.Context;
using Microsoft.EntityFrameworkCore;

namespace Chatty.Infrastructure.Repositories;
public class ChattyBrowserSessionRepository : IChattyBrowserSessionRepository
{
    private readonly ChattyDbContext _context;

    public ChattyBrowserSessionRepository(ChattyDbContext context)
    {
        _context = context;
    }

    public async Task<ChattyBrowserSession> AddAsync(ChattyBrowserSession chattyBrowserSession)
    {

        chattyBrowserSession.CreatedAt = CommonUtilityFunctions.GetCurrentInGivenTimeZone("Asia/Kolkata");
        chattyBrowserSession.CreatedAtGmt = CommonUtilityFunctions.GetCurrentInGivenTimeZone("GMT");
        chattyBrowserSession.UpdatedAt = CommonUtilityFunctions.GetCurrentInGivenTimeZone("Asia/Kolkata");
        chattyBrowserSession.UpdatedAtGmt = CommonUtilityFunctions.GetCurrentInGivenTimeZone("GMT");

        var result = await _context.ChattyBrowserSessions.AddAsync(chattyBrowserSession);
        await _context.SaveChangesAsync();
        return result.Entity;

    }

    public async Task<ChattyBrowserSession?> DeleteByIdAsync(int chattyBrowserSessionId)
    {
        var result = await _context.ChattyBrowserSessions.FirstOrDefaultAsync(b => b.Id == chattyBrowserSessionId && !b.Deleted);
        if (result != null)
        {
            _context.ChattyBrowserSessions.Remove(result);
            await _context.SaveChangesAsync();
            return result;
        }
        return null;
    }

    public async Task<IEnumerable<ChattyBrowserSession>> GetAllAsync()
    {
        return await _context.ChattyBrowserSessions.Where(b => !b.Deleted).ToListAsync();
    }

    public async Task<ChattyBrowserSession?> GetByIdAsync(int chattyBrowserSessionId)
    {
        return await _context.ChattyBrowserSessions.Where(b => !b.Deleted && b.Id == chattyBrowserSessionId).FirstOrDefaultAsync();
    }

    public async Task<ChattyBrowserSession?> UpdateAsync(ChattyBrowserSession chattyBrowserSession)
    {
        var result = await _context.ChattyBrowserSessions.FirstOrDefaultAsync(
            b => b.Id == chattyBrowserSession.Id && !b.Deleted
        );

        if (result == null)
        {
            return null;
        }

        result.SessionUuid = chattyBrowserSession.SessionUuid;

        result.UpdatedAt = CommonUtilityFunctions.GetCurrentInGivenTimeZone("Asia/Kolkata");
        result.UpdatedAtGmt = CommonUtilityFunctions.GetCurrentInGivenTimeZone("GMT");

        await _context.SaveChangesAsync();

        return result;

    }
}
